...
const deleteUser = async (req, res) => {
    if (req.cookies.user_id) {
        ...
        try {
            // First delete user's blogs
            const respBlogDelete = await Blog.deleteMany({ author: user_id });

            // Then delete the user
            const respUserDelete = await User.deleteOne({ _id: user_id });

            if (respUserDelete.deletedCount === 1) {
